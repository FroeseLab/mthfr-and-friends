---
title: "Scrip_SAINT_output_analysis_v2"
...
plot_SP_FCA <- ggplot(data = data_SAINT_merged, aes(x = SP, y = FCA, label = PreyGene)) +
  ...
  geom_text_repel(
    data = filter(
      data_SAINT_merged,
      PreyGene %in% c("MTHFR", "MTHFD1", "GCN1", "PRKDC", "FASN")
    ),
...
# Save the plot
ggsave(
  "data/output/plot_SP_FCA_check.png",
  plot = plot_SP_FCA,
  height = 2000,
  width = 2000,
  units = "px"
...
# Save the facet plot
ggsave(
  "data/output/plot_SP_FCA_facet.png",
  plot = plot_SP_FCA,
  height = 2000,
  width = 4000,
  units = "px"
...
plot_SP_FCA <- ggplot(data = filter(data_SAINT_merged, SP > 0.95, FCA > 2), aes(x = SP, y = FCA)) +
  ...
  geom_text_repel(
    aes(label = PreyGene),
    size = 2.5, nudge_y = 1, segment.size = 0.2,
    arrow = arrow(length = unit(0.01, "npc")), box.padding = 0.2, point.padding = 0.3,
...
ggsave(
  "data/output/plot_SP_0.95_FCA_2.png",
  plot = plot_SP_FCA,
  height = 2000,
  width = 2000,
  units = "px"
...
```{r}
# Set up plot settings for heatmap
plot_heatmap <- ggplot(data = filter(data_SAINT_merged, FDR <= 0.1, FCA >= 2),
  ...
  scale_fill_gradient(low = "blue", high = "red",
                      guide = guide_colorbar(direction = "horizontal", title.position = "top", title.hjust = 0.5,
                                             label.position = "bottom", ticks = TRUE)) +
  scale_x_discrete(expand = c(0, 0)) +  # Remove space on the x-axis
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = -45, hjust = 0),  # Tilt x-axis title
    legend.position = "top", legend.title = element_text(angle = 0, vjust = 1),
    legend.text = element_text(angle = 0), panel.background = element_rect(fill = "white"),  # Gray background
    panel.grid.major = element_blank(),  # No major grid lines
    panel.grid.minor = element_blank(),  # No minor grid lines
    panel.border = element_rect(color = "black", fill = NA),  # Box around the plot
    axis.title.x = element_blank(), axis.title.y = element_blank()
  )

# Generate different plots visualizing different SAINT results
plot_heatmap_FCA <- plot_heatmap + geom_tile(aes(fill = FCA))
plot_heatmap_SP <- plot_heatmap + geom_tile(aes(fill = SP))
plot_heatmap_Abundance <- plot_heatmap + geom_tile(aes(fill = Abundance))
...
```{r}
# Save the heatmaps
ggsave("data/output/plot_heatmaps.png", plot = plot_heatmaps, height = 2000, width = 3000, units = "px")
...
# Kinases
data_kinases <- read.csv("../data/raw/uniprot_kinases.csv",
                         header = TRUE,
                         check.names = FALSE)
data_SAINT_merged_modified <- data_SAINT_merged_modified %>%
  mutate(kinase = PreyGene %in% data_kinases$gene_name)
